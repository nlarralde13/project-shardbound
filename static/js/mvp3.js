...
import { ALL_BIOME_KEYS, randomTitleFor, BIOMES, BIOME_COLORS, canonicalBiome } from './biomeRegistry.js';
...
(function(){
  ...
  const NOCLIP = DEV_MODE && QS.has('noclip'); // guarded noclip

  // Persisted user token
  const USER_TOKEN = (()=>{
    const k='mvp3_user_token';
    const q=QS.get('token'); if(q){ localStorage.setItem(k,q); return q; }
    let v=localStorage.getItem(k); if(!v){ v=crypto.getRandomValues(new Uint32Array(4)).join('-'); localStorage.setItem(k,v); }
    return v;
  })();
  ...
  // Overlay instance — only ONE (removed duplicate)
  ...
  function log(text, cls=''){
    if(!consoleEl) return; const line=document.createElement('div');
    line.className='line '+(cls||''); line.textContent=text;
  ...
  function setShardStatus(msg, kind='info'){
    if(!shardStatus) return; const color = kind==='error'?'#e57373': kind==='warn'?'#ffb74d':'#9e9e9e';
    shardStatus.textContent = msg||''; shardStatus.style.color = color;
  }
  function assertCanonicalGrid(grid){ if(warnedNonCanonical) return; const bad=new Set();
    for(let y=0;y<grid.length;y++) for(let x=0;x<grid[0].length;x++) if(!ALLOWED_BIOMES.has(grid[y][x])) bad.add(grid[y][x]);
    if(bad.size){ warnedNonCanonical=true; console.warn('[Shard] Non-canonical biomes:', [...bad]); }
  }
  function biomeAt(x,y){ if(!shard) return 'Forest'; const [W,H]=shard.size||[0,0]; if(x<0||y<0||x>=W||y>=H) return 'Coast'; return shard.grid?.[y]?.[x] ?? 'Coast'; }
  ...
  function setRoom(next){
    const b=next.biome; roomTitle.textContent=next.title||'Unknown Place';
    roomBiome.textContent=b; applyArt(b);
  ...
  function siteAt(x,y){
    return shard?.sites?.find(s => (s.pos?.[0] ?? s.x) === x && (s.pos?.[1] ?? s.y) === y) || null;
  }

  // === Movement plumbing via rules ===
  function afterMoveEffects() {
    overlay.setPos(pos.x, pos.y);
    // lightweight room message on POI sighting
  ...
  async function tryMove(dir){
    ...
    if (!res.ok) {
      if (res.reason) log(res.reason, 'log-note');
      return false;
    }

    // Apply costs
    const staCost = res.costs?.sta ?? 0;
    if (staCost > 0) {
      actor.sta = Math.max(0, actor.sta - staCost);
      refreshBars();
      if (staCost >= 2) log('That terrain slows you down.', 'log-note');
    }
  ...
  function tryInteract(){
    ...
    // For now just narrate; you can wire a proper scene/router later
  ...
  // Action buttons (click UI still supported)
  ...
  // === REMOVE inline keyboard handler; use controller instead ===
  const controller = createMovementController({
    ...
    onHotkey: {
      ...
      escape:()=>{
        toggle(overlayMapEl,false);
        toggle(overlayChar,false);
        toggle(overlayInv,false);
      }
    },
  ...
  // Shard loader
  async function fetchShardList(){ try{ const res=await fetch('/api/shards'); if(!res.ok) throw new Error('HTTP '+res.status); return await res.json(); }catch{ return null; } }
  function labelForItem(it){ const disp=it?.meta?.displayName || it?.meta?.name; return disp?`${disp} (${it.file})`:it.file; }
  async function populateShardPicker(items){ if(!shardSelect) return; if(!Array.isArray(items)||!items.length) return; const seen=new Set(Array.from(shardSelect.options).map(o=>o.value)); items.forEach(it=>{ if(seen.has(it.path)) return; const opt=document.createElement('option'); opt.value=it.path; opt.dataset.name=it.file; opt.textContent=labelForItem(it); shardSelect.appendChild(opt); }); }
  ...
  async function loadShard(url){
    ...
    // Build/refresh movement rules for this shard
  ...
  btnLoadShard?.addEventListener('click', async()=>{ const url=shardSelect?.value; if(!url) return; try{ await loadShard(url); }catch(e){ console.warn(e); setShardStatus('Load failed','error'); } });
  ...
  (async()=>{
    try{
      ...
      const current = shardSelect?.value || document.getElementById('starterShardOption')?.value || '/static/public/shards/00089451_test123.json';
