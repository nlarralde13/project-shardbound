// biomeRegistry.js — Master biome registry (canonical keys + aliases + art)
// Keeps original exports. Adds art paths from /static/assets/biomes and optional frames.

const gradient = (a, b) => `linear-gradient(180deg, ${a} 0%, ${b} 100%)`;

// Canonical biome definitions (unchanged)
...
// Aliases → canonical (unchanged, with a couple more)
export const ALIASES = {
  ocean:'Ocean', sea:'Ocean', deep_ocean:'Ocean',
  coast:'Coast', beach:'Coast', shore:'Coast', ocean_port:'Coast',
  ...
  grassland:'Plains', prairie:'Plains', steppe:'Plains', plains:'Plains',
  savanna:'Savanna', savannah:'Savanna',
  shrubland:'Shrubland', scrub:'Shrubland', chaparral:'Shrubland',

  forest:'Forest', woodland:'Forest', dark_forest:'Forest',
...
// Back-compat "BIOMES" object used by the viewer (tint + optional art/frames)
export const BIOMES = Object.fromEntries(
  Object.entries(BIOME_MASTER).map(([k, v]) => [
    k,
    { tint: gradient(v.colors.uiTintStart, v.colors.uiTintEnd), art: null, art_combat: null, frames: null, frame_ms: 120 }
  ])
);
...
const OCEAN_FRAMES = Array.from({ length: 8 }, (_, i) =>
  asset(`webm/ocean_frames/ocean_frame_${i + 1}.png`)
);

function setArt(key, { art, combat, frames, frame_ms }) {
  if (!BIOMES[key]) return;
  if (art) BIOMES[key].art = art;
  if (combat) BIOMES[key].art_combat = combat;
  if (frames) BIOMES[key].frames = frames;
  if (frame_ms) BIOMES[key].frame_ms = frame_ms;
}

// Map the assets you actually have today
setArt('Plains',     { art: asset('plains.png') });
setArt('Forest',     { art: asset('dark_forest.png') });       // placeholder for forest/taiga
setArt('Tundra',     { art: asset('tundra.png') });
setArt('DesertSand', { art: asset('desert.png') });
setArt('Volcano',    { art: asset('volcano.png') });
setArt('Urban',      { art: asset('town.png') });              // settlements / city feel
setArt('Coast',      { art: asset('ocean_port.png') });        // nice coastal placeholder

// Ocean gets a gentle animated frameset if you want to use it
setArt('Ocean', { frames: OCEAN_FRAMES, frame_ms: 120 });
...
// Convenience: art accessors (optional)
export function artFor(biome, mode = 'idle') {
  const key = canonicalBiome(biome);
  const b = BIOMES[key];
  return mode === 'combat' ? (b.art_combat || b.art) : b.art;
}
export function framesFor(biome) {
  const key = canonicalBiome(biome);
  return BIOMES[key]?.frames || null;
}
