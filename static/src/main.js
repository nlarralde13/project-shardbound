...
window.addEventListener('DOMContentLoaded', async () => {
  ...
  const btn = document.getElementById('shardEditor');
  if (btn) {
    btn.addEventListener('click', () => {
      const url = new URL('/static/src/dev/shard-editor.html', location.origin).toString();
      const w = window.open(url, '_blank');   // must be sync in the click
  if (w) {
    // ensure no window.opener reference
